...
const emit = defineEmits(['editar-articulo']);
const articuloStore = useArticuloStore();

onMounted(() => {
  articuloStore.obtenerArticulos();
});

const manejarEliminar = async (id: number) => {
  if (confirm('¿Estás seguro de que quieres eliminar este artículo?')) {
    await articuloStore.eliminarArticulo(id);
  }
};

const editarArticulo = (articulo: Articulo) => {
  emit('editar-articulo', articulo);
};
</script>
...
<template>
  <div class="articulo-list">
    ...
    <div v-if="articuloStore.cargando">Cargando...</div>
    ...
    <table v-else>
      ...
      <tbody>
        <tr v-for="articulo in articuloStore.listaArticulos" :key="articulo.id">
          <td>{{ articulo.id }}</td>
...
.badge {
  background-color: rgb(11, 201, 21);
  padding: 2px 6px;
  border-radius: 10px;
  margin-right: 5px;
  font-size: 0.8em;
