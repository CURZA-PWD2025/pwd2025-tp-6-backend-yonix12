...
const emit = defineEmits(['editar-proveedor']);
...
const manejarEliminar = async (id: number) => {
  if (confirm('¿Estás seguro de que quieres eliminar este proveedor?')) {
    await proveedorStore.eliminarProveedor(id);
  }
};

const editarProveedor = (proveedor: Proveedor) => {
  emit('editar-proveedor', proveedor);
};
</script>
...
<template>
  <div class="proveedor-list">
    ...
    <div v-if="proveedorStore.cargando">Cargando...</div>
    ...
    <table v-else>
      ...
      <tbody>
        <tr v-for="proveedor in proveedorStore.listaProveedores" :key="proveedor.id">
          <td>{{ proveedor.id }}</td>
...
th, td { border: 1px solid #ddd; padding: 28px; text-align: left; }
